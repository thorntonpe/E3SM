module lnd_import_export_bypass
...
contains
  ...
  subroutine lnd_import( bounds, x2l, atm2lnd_vars, glc2lnd_vars, lnd2atm_vars)
    ...
    use lnd_disagg_forc
    use lnd_downscale_atm_forcing_bypass
    use netcdf
    !
    ! !ARGUMENTS:
    ...
    do g = bounds%begg,bounds%endg
       ...
       atm2lnd_vars%deficit_grc(g) = x2l(index_x2l_Flrr_deficit,i)
        ...
        ! Determine required receive fields (bypass the coupler)
...
10          continue
   ...
   !------------------------------------Aerosol forcing--------------------------------------------------
       ...
       do topo = grc_pp%topi(g), grc_pp%topf(g)
         ...
         if (use_atm_downscaling_to_topunit) then
           ...
           call downscale_atm_forcing_to_topounit(g, i, lnd2atm_vars, atm2lnd_vars)
         end if
       end do
  ...
  !-----------------------------------------------------------------------------------------------------
       ...
       if (co2_type_idx == 1) then
        ...
        !Bypass: atmospheric CO2 (to be used for transient simulations only)
        if (atm2lnd_vars%loaded_bypassdata .eq. 0) then 
...
end module lnd_import_export_bypass
