module lnd_downscale_atm_forcing_bypass
...
contains
    ...
    !-----------------------------------------------------------------------
  subroutine downscale_atm_forcing_to_topounit(g, i, lnd2atm_vars, atm2lnd_vars)
    !
    ! !DESCRIPTION:
    ...
    !uovern_t    = x2l(index_x2l_Sa_uovern,i)     ! Froude Number  (NOTE DMR 2/1/2023 - need to figure out how to obtain for bypass)
    
    snow_g      = atm2lnd_vars%forc_snow_not_downscaled_grc(g) 
    rain_g      = atm2lnd_vars%forc_rain_not_downscaled_grc(g)  
    ...
    if (numt_pg > 1) then          !downscaling is done only if a grid has more than 1 topounits 
       ...
       do t = grc_pp%topi(g), grc_pp%topf(g)       
          ...
          if (other_forcing_dwn == 0) then
             call downscale_atm_state_to_topounit(g, i, t, lnd2atm_vars, atm2lnd_vars, uaflag)
             call downscale_longwave_to_topounit(g, i, t, lnd2atm_vars, atm2lnd_vars, uaflag)
    ...
    if (numt_pg > 1) then
          ...
          call build_normalization(orig_field=atm2lnd_vars%forc_q_not_downscaled_grc(g), &
       ...
       call build_normalization(orig_field=atm2lnd_vars%forc_lwrad_not_downscaled_grc(g), &
  ...
  !-----------------------------------------------------------------------
  subroutine downscale_atm_state_to_topounit(g, i, t, lnd2atm_vars, atm2lnd_vars, method)
    !
    ! !DESCRIPTION:
    ...
    tbot_g = atm2lnd_vars%forc_t_not_downscaled_grc(g)
    thbot_g = atm2lnd_vars%forc_th_not_downscaled_grc(g)
    ...
    qbot_g = atm2lnd_vars%forc_q_not_downscaled_grc(g)
    pbot_g = atm2lnd_vars%forc_pbot_not_downscaled_grc(g)
    zbot_g = atm2lnd_vars%forc_hgt_grc(g)
...
!    call check_downscale_consistency(bounds, atm2lnd_vars)
  ...
  subroutine downscale_longwave_to_topounit(g, i, t, lnd2atm_vars, atm2lnd_vars, method)
...
end module lnd_downscale_atm_forcing_bypass
