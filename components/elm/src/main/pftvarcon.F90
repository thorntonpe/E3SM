...
!-----------------------------------------------------------------------
    ...
    if ( .not. readv ) call ncd_io('mergetoclmpft', mergetoelmpft, 'read', ncid, readvar=readv)
    if ( .not. readv ) then
       do i = 0, mxpft
          mergetoelmpft(i) = i
       end do
    ...
    if ( .not. readv ) needleleaf(:) = 0   ! will assign a value below
    ! if 'climatezone' flag is defined for each PFT
    call ncd_io('climatezone', needleleaf, 'read', ncid, readvar=readv)
    if ( .not. readv ) climatezone(:) = 0  ! will assign a value below
    ! if 'nfixer' flag is defined for each PFT (now as a placeholder)
    call ncd_io('nfixer', nfixer, 'read', ncid, readvar=readv)
    if ( .not. readv ) nfixer(:) = 0
